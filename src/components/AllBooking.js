...
export default function AllBooking() {
  ...
  useEffect(() => {
    axios.get("https://peaceful-beach-25534.herokuapp.com/api/v2/booking").then((res) => {
      if (res.data == 0) {
        console.log("no data");
      } else {
  ...
  function clickDelete(id) {
    if (window.confirm("are you sure you want to delete the booking?")) {
      axios.delete("https://peaceful-beach-25534.herokuapp.com/api/v2/booking" + id).then((res) => {
        // console.log(res.data);
  ...
  return (
    <>
      ...
      {!token && !role ? (
        (window.location = "https://peaceful-beach-25534.herokuapp.com/api/v2/admin/login")
      ) : (
        <div>
          <img
            class="fillpic"
